...
import { Container, Loader2, CheckCircle2, AlertTriangle, X, Layers } from 'lucide-react';
...
export const Step2: React.FC<Props> = ({
  ...
  pipelines:
    ...
    {/* Namespace recipe — always visible, independent of Smart-add / the
        compact-after-apply state above. Each app needs a distinct
        service.namespace to land as its own OTel Namespace and roll up to a
        Business Service in AIOps; the shared X-Source can't separate them.
        This is app-side config, so we document it rather than edit it for the
        user (the resource processor lives under `processors:`, which smart-add
        never touches). */}
...
OTEL_RESOURCE_ATTRIBUTES=service.namespace=<app>,deployment.environment=<env>`} />
      <p className="text-tiny text-gray-500 -mt-4">
        ...
        <code className="font-mono">resource</code> processor to its collector instead. To bind namespaces to a Business
        Service in AIOps, see the <span className="text-gray-300">Onboarding multiple applications</span> section of the
        README.
      </p>
    </div>
