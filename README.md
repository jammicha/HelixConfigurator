...
[Onboarding multiple applications](#onboarding-multiple-applications) covers grouping several apps under one Helix tenant via `service.namespace`. When you instead need to send telemetry to more than one Helix tenant from the same gateway (separate production and staging tenants, or two customer environments, for instance), use **Manage Connections**: open it from the avatar menu, or from the connection switcher in the Gateway Dashboard header once a second connection exists.

- **Named connections.** Each connection has its own display name, endpoint, API key, and X-Source, one per Helix tenant. Add, edit, or remove them from the Manage Connections page; the form runs the same validation as the onboarding wizard's Step 1 (endpoint format, three-part API key, X-Source character rules).
- **Per-signal fan-out.** Each connection independently toggles traces, metrics, and logs. Turning a signal off for a connection removes that connection's exporter from that signal's pipeline entirely, so, for example, a metrics-only connection sends no traces or logs at all.
- **Active connection.** Exactly one enabled connection is active at a time. Single-tenant features that have no per-signal notion of "which tenant" (Situations and Events, the AIOps Business Service link, container/service deep-links, and the diagnostics panel) operate against the active connection, and so does **Generate Kubernetes deployment**: the chart is built from the active connection's endpoint and key. Switch which connection is active from the connection switcher in the Gateway Dashboard header (shown once 2+ connections exist); each connection's health (healthy, failing, idle, or disabled) is shown there and on the Manage Connections page, drawn from that connection's own exporter counters.
- **Scoped reset.** The reset dialog lets you choose which connections to reset instead of always wiping everything. Selecting a subset removes just those connections and leaves the rest, plus your wizard progress, alone. Selecting every connection (the default when the dialog opens) is the same full onboarding reset as before: it clears the bare env keys, drops bridged networks, and returns you to Step 1.

**Behind the scenes:** each enabled connection becomes a managed exporter block named `otlphttp/bmchelix_<id>` in `helix-otel-collector.yaml` (`<id>` is the connection's slug, so a connection named "EU Prod" becomes `otlphttp/bmchelix_eu-prod`). The configurator regenerates only these managed blocks and their membership in each pipeline's exporters list, using targeted line edits rather than a full YAML parse and re-emit. Everything else in the file, including receivers, processors, the local-viewer exporter, any exporter you added by hand, and comments, is preserved exactly as you wrote it. A connection is wired into only the pipelines for the signals it has enabled, so a connection with metrics disabled has no entry in the `metrics` pipeline at all.

Credentials and endpoints for these exporters live in per-connection env keys (`HELIX_ENDPOINT_<SUF>`, `HELIX_API_KEY_<SUF>`, `X_SOURCE_<SUF>`), and the active connection is also mirrored onto the bare `HELIX_ENDPOINT` / `HELIX_API_KEY` / `X_SOURCE` / `BUSINESS_SERVICE_KEY` / `HELIX_EVENTS_ENDPOINT` keys that the single-tenant features above read. See `.env.example` for the exact key shapes; both sets are managed by the configurator and should not be hand-edited.
