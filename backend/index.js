...
// --- Convert a trace into an event in BMC Helix AIOps ---------------------
// The Events API is the only ingress we need: the customer's correlation
// policy in AIOps is what aggregates events into Situations. We don't (and
// shouldn't) create Situations directly — that's a downstream effect.
// Endpoint reference:
//   docs.bmc.com/docs/helixoperationsmanagement/241/en/event-management-...
// Auth: same API key the OTel collector uses, but in the Authorization
// header with the `apiKey` scheme instead of `X-Api-Key`.
